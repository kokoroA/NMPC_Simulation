...
class Params:
    ...
    # Quad-X: u_hover_nom = mg/(4·T_max) ≈ 0.180、ヘキサの 0.120 より高い
    # 旧 pwm_ascent=0.125 (ヘキサ余剰 +0.083N) と同等の余剰にする:
    #   T_max·4·pwm_ascent − mg = 0.083 → pwm_ascent ≈ 0.188
    # Quad-X: PKG-E2 (mass+5%, T_max-5%) + PKG-E3 (aero drag) を勘案して上方修正
    # 必要 duty ≈ mg·1.05 / (4·T_max·0.95) ≈ 0.199 (静止) + aero マージン
    pwm_ascent: float = 0.215         # 0.188→0.215: PKG-E のプラント誤差 + aero マージン
    ...
    Q_q: float = 1.0
    Q_r: float = 1.0
    R_u: float = 21.0              # 14 × (6/4) ≈ 21 (Quad はモータ当たり制御権限が大)
    ...
    # ---- PKG-E5: プラント状態次元 (NX_plant=11 で水平移動 x,y,u_v,v_v 追加) ----
    NX_plant: int = 11
    ...
    def u_hover_nom(self) -> float:
        """1 モータ当たりのホバリング定常入力 = mg / (4 · T_max).

        Quad-X: 0.200 × 9.81 / (4 × 2.725) = 0.180
        """
        return self.mass * self.gravity / (4.0 * self.T_max)


# =========================================================================
# 2. QuadModel  (連続時間ダイナミクス + Jacobian + 離散化)
# =========================================================================
class QuadModel:
    """
    Quad-X 配置の姿勢ダイナミクス。
        状態 x = [phi, theta, p, q, r]
        入力 u = [u_M1..u_M4] in [0,1]
            M1: 前右 (FR)   ( +L/√2, +L/√2 )   CCW  s = +1
            M2: 後左 (RL)   ( -L/√2, -L/√2 )   CCW  s = +1
            M3: 前左 (FL)   ( +L/√2, -L/√2 )   CW   s = -1
            M4: 後右 (RR)   ( -L/√2, +L/√2 )   CW   s = -1
        トルク:
            tau_x_i = -y_i · T_max · u_i
            tau_y_i = +x_i · T_max · u_i
            tau_z_i = s_i · C_QT · T_max · u_i

    対称性検証: B_c · 1 (全 4 モータに均等 duty) が 3 軸とも 0
        → 平均モード(= 高度成分)と差分モード(= 姿勢成分)が分離
        → PKG-C 加算合成が成立
    冗長性:
        rank(B_c[2:5, :]) = 3 だが NU = 4 なので null space は 1 次元のみ
        ヘキサ(null space 3 次元)より大幅に貧弱
        → 完全故障(η=0)は B_eff のランクが 2 に落ち、3 軸制御不可能
        → 部分故障(η > 0)では rank 維持で対処可
    """
    def __init__(self, p: Params):
        self.p = p
        # Quad-X: 45°, -135°, -45° (=315°), 135°  (ArduPilot/PX4 標準)
        angles_deg = np.array([45.0, -135.0, -45.0, 135.0])
        s_yaw      = np.array([+1.0, +1.0, -1.0, -1.0])  # CCW=+1, CW=-1
        ...
        self.eta = np.ones(4, dtype=np.float64)
        self._build_Bc()

    def _build_Bc(self) -> None:
        p = self.p
        Bc = np.zeros((5, 4), dtype=np.float64)
        for k in range(4):
    ...
    def set_eta(self, eta: np.ndarray) -> None:
        ...
        self.eta[:] = np.asarray(eta, dtype=np.float64).reshape(4)
        self._refresh_Bc_eff()
...
class PlantModel:
    ...
    プラント側の真モデル。状態 7 次元:
        x = [phi, theta, p, q, r, z, w]
            ...
            w : NED 下向き正 [m/s]
    NMPC の姿勢ダイナミクスは QuadModel をそのまま流用。
    ...
    def __init__(self, p: Params):
        ...
        # ---- PKG-E2: プラント側は誤差を含む物理パラメータで QuadModel を構築 ----
        p_plant = copy.copy(p)
        ...
        self.attitude = QuadModel(p_plant)
    ...
    def f_full(self,
        ...
        プラント状態 (PKG-E5):
            ...
            7 次元渡された場合は自動で 11 次元にパディング (後方互換)。
        """
        p = self.p_plant   # PKG-E2: 真値のプラントパラメータを使う
        x_plant = np.asarray(x_plant, dtype=np.float64)
        if x_plant.size == 7:
            x_plant = np.concatenate([x_plant, np.zeros(4, dtype=np.float64)])
        ...
        if self.p.J_prop_eq > 1e-12:
            ...
            pr = float(x_plant[2]); qr = float(x_plant[3])
            xdot5[2] += (-H_prop * qr) / p.Ix
            xdot5[3] += (+H_prop * pr) / p.Iy

        # ---- 推力計算 (E3 空力 + E7 地面効果) ----
        phi   = float(x_plant[0])
        theta = float(x_plant[1])
        z     = float(x_plant[5])
        w     = float(x_plant[6])
        u_vel = float(x_plant[9])
        v_vel = float(x_plant[10])
        ...
        # ---- 並進 (NED, yaw=0 仮定) ----
        a_x = -(T_total / p.mass) * sth + F_world[0] / p.mass
        a_y =  (T_total / p.mass) * sphi * cth + F_world[1] / p.mass
        a_z =  p.gravity - (T_total / p.mass) * cphi_cth + F_world[2] / p.mass

        xdot = np.zeros(11, dtype=np.float64)
        xdot[:5] = xdot5
        xdot[5]  = w           # zdot
        xdot[6]  = a_z         # wdot
        xdot[7]  = u_vel       # x_dot
        xdot[8]  = v_vel       # y_dot
        xdot[9]  = a_x         # u_vel_dot
        xdot[10] = a_y         # v_vel_dot
        return xdot
    ...
    def step_rk4(self,
                 ...
                 tau_body: np.ndarray,
                 dt: float) -> np.ndarray:
        """1 ステップ RK4 (PKG-E5 で 11 次元対応, 7 次元入力は自動パディング)."""
        x_plant = np.asarray(x_plant, dtype=np.float64)
        if x_plant.size == 7:
            x_plant = np.concatenate([x_plant, np.zeros(4, dtype=np.float64)])
...
class NoiseModel:
    ...
    def apply_process(self, x_true: np.ndarray, dt: float) -> np.ndarray:
        ...
        if x.size >= 7 and p.sigma_w_proc > 0.0:
            x[6] += self.rng.normal(0.0, p.sigma_w_proc * sdt)
        return x
    ...
    def observe(self, x_true: np.ndarray) -> np.ndarray:
        ...
        # 高度 (idx 5, 6) — 7 次元以上のときのみ
        if x_true.size >= 7:
            if p.alt_delay_steps > 0:
                self._alt_delay.append(x_true[5:7].copy())
                alt_delayed = self._alt_delay[0].copy()
            else:
                alt_delayed = x_true[5:7].copy()
        else:
            alt_delayed = None

        # 観測ベクトル組み立て (姿勢以外の高次元状態 x_true[7:] はそのままパススルー)
        if x_true.size >= 7:
            x = np.concatenate([imu_delayed, alt_delayed, x_true[7:].copy()])
        else:
            x = imu_delayed.copy()
        ...
        if x.size >= 7:
            if p.sigma_z_meas > 0.0:
                x[5] += self.rng.normal(0.0, p.sigma_z_meas)
            if p.sigma_w_meas > 0.0:
                x[6] += self.rng.normal(0.0, p.sigma_w_meas)
        return x


# =========================================================================
# 3. AdmmNmpcQuad  (凝縮形 NMPC + SR2 + ADMM)
# =========================================================================
class AdmmNmpcQuad:
    """sample/nmpc.cpp の Python 鏡像."""

    def __init__(self, model: QuadModel, p: Params):
        self.model = model
        self.p = p
    ...
    def _full_rebuild_chol(self) -> None:
        ...
        if not ok:
            raise np.linalg.LinAlgError(
                "AdmmNmpcQuad: Cholesky failed even with ridge fallback")
    ...
    def step(self,
        ...
        """
        PKG-C: u_baseline (NU,) を渡すと加算合成モード。
        ...
        if u_baseline is not None:
            self.u_baseline = np.asarray(u_baseline, dtype=np.float64).reshape(p.NU)
        else:
...
class Logger:
    ...
    def log(self, t, x, u_cmd, u_app, eta, iter_us, was_reset, x_ref,
        ...
        # 5 次元状態が来た場合は z=w=0 でパディング
        xa = np.array(x, dtype=np.float64)
        if xa.size == 5:
            xa = np.concatenate([xa, np.zeros(2)])
        self.x.append(xa)
        if x_meas is None:
            self.x_meas.append(xa.copy())
        else:
            xm = np.array(x_meas, dtype=np.float64)
            if xm.size == 5:
                xm = np.concatenate([xm, np.zeros(2)])
    ...
    def plot(self, title: str = "", savepath: Optional[str] = None):
        ...
        x_data = d['x']
        if x_data.ndim >= 2 and x_data.shape[1] >= 11:
            final_x = float(x_data[-1, 7])
            final_y = float(x_data[-1, 8])
            max_dist = float(np.max(np.sqrt(x_data[:, 7] ** 2 + x_data[:, 8] ** 2)))
        ...
        # 行2: コマンド入力 (NMPC 出力) Quad-X: M1..M4 を 2+2+全重ね
        axs[2, 0].plot(t, u_cmd[:, 0], label='M1 cmd (FR)', color='C0')
        axs[2, 0].plot(t, u_cmd[:, 1], label='M2 cmd (RL)', color='C1')
        axs[2, 0].set_ylabel('u_cmd M1,M2 (CCW)')
        axs[2, 0].set_ylim(-0.05, 1.05)

        axs[2, 1].plot(t, u_cmd[:, 2], label='M3 cmd (FL)', color='C0')
        axs[2, 1].plot(t, u_cmd[:, 3], label='M4 cmd (RR)', color='C1')
        axs[2, 1].set_ylabel('u_cmd M3,M4 (CW)')
        axs[2, 1].set_ylim(-0.05, 1.05)

        for k in range(4):
            axs[2, 2].plot(t, u_cmd[:, k], label=f'M{k+1}', lw=0.7)
        axs[2, 2].set_ylabel('u_cmd all 4')
        ...
        # 行3: 適用入力 M1..M4 + η + サチュ率
        axs[3, 0].plot(t, u_app[:, 0], label='M1 (FR, CCW)', color='C0')
        axs[3, 0].plot(t, u_app[:, 1], label='M2 (RL, CCW)', color='C1')
        axs[3, 0].set_ylabel('u_app M1,M2 (CCW)')
        axs[3, 0].set_xlabel('t [s]')
        axs[3, 0].set_ylim(-0.05, 1.05)

        axs[3, 1].plot(t, u_app[:, 2], label='M3 (FL, CW)', color='C0')
        axs[3, 1].plot(t, u_app[:, 3], label='M4 (RR, CW)', color='C1')
        axs[3, 1].set_ylabel('u_app M3,M4 (CW)')
        ...
        axs[4, 0].plot(t, -d['x'][:, 5], label='altitude (-z)', color='C0')
        ...
        # (4,1) 鉛直速度 w (NED 下向き正)
        axs[4, 1].plot(t, d['x'][:, 6], label='w [m/s]', color='C0')
...
def sanity_checks(params: Params) -> None:
    ...
    # M1 (前右, x=+L/√2, y=+L/√2): B_c[2,0] = -y_M1 · T_max / I_x = -(L/√2)·T_max/I_x  → 負
    L_over_sqrt2 = model.p.arm_length / math.sqrt(2.0)
    expected_phi_M1 = -L_over_sqrt2 * model.p.T_max / model.p.Ix
    assert abs(model.Bc[2, 0] - expected_phi_M1) < 1e-6, \
        f"B_c[2,0] expected {expected_phi_M1}, got {model.Bc[2,0]}"

    # 2) M1 (前右) のみ duty=1: y_M1>0 → ṗ<0 (右下げ), x_M1>0 → q̇>0, CCW → ṙ>0
    u = np.zeros(4); u[0] = 1.0
    xd = model.f(x0, u)
    print(f"  [M1 (前右) only] xdot = {xd}")
    assert xd[2] < 0, "M1 (前右, y>0) → pdot<0 を期待"
    assert xd[3] > 0, "M1 (前右, x>0) → qdot>0 を期待"
    assert xd[4] > 0, "M1 CCW → rdot>0 を期待"

    # 3) M3 (前左, CW) のみ duty=1: y_M3<0 → ṗ>0, x_M3>0 → q̇>0, CW → ṙ<0
    u = np.zeros(4); u[2] = 1.0
    xd = model.f(x0, u)
    print(f"  [M3 (前左) only] xdot = {xd}")
    assert xd[2] > 0, "M3 (前左, y<0) → pdot>0 を期待"
    assert xd[3] > 0, "M3 (前左, x>0) → qdot>0 を期待"
    assert xd[4] < 0, "M3 CW → rdot<0 を期待"

    # 4) ホバートリム: 全 4 機 u_hover_nom → 角加速度 ≈ 0
    u = np.full(4, params.u_hover_nom)
    xd = model.f(x0, u)
    print(f"  [hover all-4] xdot[2..4] = {xd[2:]}  (expect ~0)")
    ...
    # 6) B_c[2:5,:] のランク = 3、null space = 1 次元 (平均モード)
    rank_attitude = np.linalg.matrix_rank(model.Bc[2:, :])
    print(f"  rank(B_c[2:5,:]) = {rank_attitude}  (期待 3, null space = NU-3 = 1 次元)")
    assert rank_attitude == 3
    _, S, Vt = np.linalg.svd(model.Bc[2:, :])
    null_vec = Vt[-1]
    print(f"  null space vector = {null_vec}  (期待: ±[0.5,0.5,0.5,0.5] 近傍)")
    assert np.all(np.abs(np.abs(null_vec) - 0.5) < 0.01), \
        f"null space が平均モードでない: {null_vec}"

    # 7) set_eta: B_eff = B_nom · diag(η) のスケーリング確認
    eta_test = np.array([1.0, 1.0, 0.5, 1.0])       # M3 部分故障
    model.set_eta(eta_test)
    ratio = model.Bc[2:, 2] / model.Bc_nom[2:, 2]
    print(f"  [set_eta η3=0.5] B_eff[:,2] / B_nom[:,2] = {ratio} (expect ~0.5)")
    assert np.allclose(ratio, 0.5)
    model.set_eta(np.ones(4))

    # 7b) 完全故障 (η=0) → 制御不能性の検証
    # 3 機残存でも B_eff[2:5,:] のランクは依然 3 (3x3 サブ行列が線形独立)。
    # しかし「ホバー (Σu = mg/T_max) かつ零モーメント (B_c·u = 0)」を 同時 に満たす
    # 解は u=0 (推力 0) のみ → 結局ホバー不可能。これがクアッド完全故障の真の制約。
    eta_fail = np.array([1.0, 1.0, 0.0, 1.0])
    model.set_eta(eta_fail)
    rank_fail = np.linalg.matrix_rank(model.Bc[2:, :])
    print(f"  [η3=0 完全故障] rank(B_eff[2:5,:]) = {rank_fail}  "
          f"(rank 3 のまま — 3 機が独立な姿勢方向を張れるため)")
    assert rank_fail == 3
    # ホバー条件 (推力配分 1·u = mg/T_max かつ B[2:]·u = 0) は非自明解を持たない:
    # 拡張行列 [B[2:]; 1] (4x3 残機) のランクが 4 になることを確認。
    B_alive = np.delete(model.Bc[2:, :], 2, axis=1)         # M3 列を除去 (3x3)
    aug = np.vstack([B_alive, np.ones((1, 3))])             # 4x3
    rank_aug = np.linalg.matrix_rank(aug)
    print(f"  [η3=0] rank([B_alive; 1^T]) = {rank_aug}  "
          f"(期待 3, つまり零モーメント+ホバー同時達成は u=0 のみ → 飛行不能)")
    assert rank_aug == 3, "ホバー+零モーメント拡張系のランクが 3 でない"
    model.set_eta(np.ones(4))
    ...
    # 9) 重力補償: phi=0.3 で T_trim = m*g / cos(0.3)
    pid.reset()
    ...
    # 11) PlantModel: ホバー条件 (4 機) で姿勢角加速度 ≈ 0
    ...
    # M3 完全故障: sum_eta=3
    eta_fail3 = np.array([1.0, 1.0, 0.0, 1.0])
    uh2 = t_cmd_to_u_hover(mg, eta_fail3, params)
    expected_per = mg / (3.0 * params.T_max)
    print(f"  [u_hover fail M3] per (alive) = {uh2[0]:.5f} (expect {expected_per:.5f}), "
          f"M3 duty = {uh2[2]:.5f} (expect 0)")
    assert abs(uh2[0] - expected_per) < 1e-6 and abs(uh2[2]) < 1e-9
    # M3 部分故障 η=0.5: Σ(η·u)·T_max = T_cmd
    eta_partial = np.array([1.0, 1.0, 0.5, 1.0])
    uh3 = t_cmd_to_u_hover(mg, eta_partial, params)
    thrust = float(np.sum(eta_partial * uh3)) * params.T_max
    print(f"  [u_hover partial η3=0.5] thrust={thrust:.5f} N (expect mg={mg:.5f})")
    assert abs(thrust - mg) < 1e-6

    # 14) PKG-C 加算合成: u_baseline = u_hover_nom·1 で Δu ≈ 0
    nmpc_test = AdmmNmpcQuad(QuadModel(params), params)
    u_base = np.full(4, params.u_hover_nom, dtype=np.float64)
    nmpc_test.set_box(
        np.full(4, params.U_min - params.u_hover_nom),
        np.full(4, params.U_max - params.u_hover_nom),
    )
    x0_hover = np.zeros(5)
    ...
    # 16) PKG-D1: u_baseline ramp 補間 (4 機ぶん)
    print("\n  [PKG-D1] u_baseline ramp 補間テスト (Quad-X)")
    u_old = np.full(4, params.u_hover_nom)                              # 故障前 4機均等
    u_new = np.array([0.241, 0.241, 0.0, 0.241])                        # M3 故障後 3機均等
    eta_after = np.array([1.0, 1.0, 0.0, 1.0])
    beta = 0.0
    u_mid = (1.0 - beta) * u_old + beta * u_new
    u_mid = np.where(eta_after > 1.0e-3, u_mid, 0.0)
    print(f"    β=0.0  u_baseline = {u_mid}  (期待: M3=0, 他は {params.u_hover_nom:.4f})")
    assert abs(u_mid[2]) < 1e-9 and abs(u_mid[0] - params.u_hover_nom) < 1e-9
    beta = 1.0
    u_mid = (1.0 - beta) * u_old + beta * u_new
    u_mid = np.where(eta_after > 1.0e-3, u_mid, 0.0)
    print(f"    β=1.0  u_baseline = {u_mid}  (期待: M3=0, 他は 0.241)")
    assert abs(u_mid[2]) < 1e-9 and abs(u_mid[0] - 0.241) < 1e-9
    beta = 0.5
    expected_mid = 0.5 * (params.u_hover_nom + 0.241)
    u_mid = (1.0 - beta) * u_old + beta * u_new
    u_mid = np.where(eta_after > 1.0e-3, u_mid, 0.0)
    print(f"    β=0.5  u_baseline = {u_mid}  (期待: M3=0, 他は {expected_mid:.4f})")
    assert abs(u_mid[2]) < 1e-9 and abs(u_mid[0] - expected_mid) < 1e-6
    ...
    x_hover = np.zeros(11); x_hover[5] = -1.0
    x_asc   = x_hover.copy(); x_asc[6] = -3.0
    ...
    print(f"    ホバー w_dot   = {xdot_h_e[6]:+.4f}")
    print(f"    上昇中 w_dot   = {xdot_a_e[6]:+.4f}  (期待: より大きな正値=推力不足)")
    if params.w_induced_hover > 1e-9:
        assert xdot_a_e[6] > xdot_h_e[6] - 1e-6, "上昇時に推力が減らない"
    ...
    if params.imu_delay_steps > 0 and params.noise_enable:
        ...
        for k in range(40):
            x_true_k = np.zeros(7)
            x_true_k[0] = 0.1 if k >= 10 else 0.0
    ...
    x_tilted = np.zeros(11); x_tilted[5] = -1.0; x_tilted[1] = 0.2
    xdot_tilt = plant_t5.f_full(x_tilted, u_h_e, np.zeros(3), np.zeros(3))
    print(f"    theta=+0.2 rad の a_x_world = {xdot_tilt[9]:+.4f} m/s² "
          f"(a_x = -T/m·sin θ なので theta>0 で負)")
    assert abs(xdot_tilt[9]) > 1e-3, "傾斜時に水平加速度が発生していない"
    x_roll = np.zeros(11); x_roll[5] = -1.0; x_roll[0] = 0.2
    xdot_roll = plant_t5.f_full(x_roll, u_h_e, np.zeros(3), np.zeros(3))
    print(f"    phi=+0.2 rad の a_y_world = {xdot_roll[10]:+.4f} m/s² "
          f"(右傾で +y 方向加速が期待)")
    assert xdot_roll[10] > 0.0, "右傾で +y 加速にならない"
    ...
    if params.ge_height > 1e-9:
        ...
        x_low  = np.zeros(11); x_low[5]  = -0.1
        x_high = np.zeros(11); x_high[5] = -2.0
        xdot_l = plant_t7.f_full(x_low,  u_h_e, np.zeros(3), np.zeros(3))
        xdot_h = plant_t7.f_full(x_high, u_h_e, np.zeros(3), np.zeros(3))
        print(f"    高度0.1m w_dot={xdot_l[6]:+.4f} (推力ブースト中)")
        print(f"    高度2.0m w_dot={xdot_h[6]:+.4f} (通常)")
        assert xdot_l[6] < xdot_h[6], "地面効果が効いていない"
    else:
...
def compute_metrics(d: dict, t_settle_thresh: float = 0.05) -> dict:
    ...
    if d['x'].shape[1] >= 7 and d['z_ref'].size > 0:
        z_err = d['x'][:, 5] - d['z_ref']
        rms_z_err = float(np.sqrt(np.mean(z_err ** 2)))
        peak_w    = float(np.max(np.abs(d['x'][:, 6])))
    else:
        rms_z_err = 0.0
        peak_w    = 0.0
...
def run_scenario(params: Params,
    ...
    plant   = PlantModel(params)
    nmpc_model = QuadModel(params)
    nmpc = AdmmNmpcQuad(nmpc_model, params)
    ...
    # 初期状態 (PKG-E5 で 11 次元): [φ, θ, p, q, r, z, w, x_pos, y_pos, u_vel, v_vel]
    # 変数名は歴史的経緯で x7_true/x7_meas を維持 (idx 0..6 は不変)
    ...
    if x0 is None:
        ...
        if x0a.size == 5:
            x7_true = np.concatenate([x0a, np.zeros(NX_PLANT - 5)])
        elif x0a.size == 7:
            x7_true = np.concatenate([x0a, np.zeros(NX_PLANT - 7)])
        elif x0a.size == NX_PLANT:
            x7_true = x0a.copy()
        else:
            raise ValueError(f"x0 must be 5/7/{NX_PLANT} dim, got {x0a.size}")
    ...
    # 入力 ZOH 保持変数
    u_cmd = np.full(params.NU, params.u_hover_nom, dtype=np.float64)
    u_hover = u_cmd.copy()
    ...
    ascent_pwm_vec = np.full(params.NU, params.pwm_ascent, dtype=np.float64)
    ...
    if phase == "control":
        ...
        z_at_bumpless_restart = float(x7_true[5])
        w_at_bumpless_restart = float(x7_true[6])
    ...
    for n in range(n_sim_steps):
        ...
        if phase == "ascent" and float(x7_true[5]) <= z_switch:
            phase = "control"
            t_switch = t
            z_at_switch = float(x7_true[5])
            w_at_switch = float(x7_true[6])         # PKG-A: bumpless ramp 用に w を保存
            alt_pid.reset()
        ...
        if n % dec == 0:
            if phase == "control":
                ...
                T_cmd, pid_info = alt_pid.update(
                    z=float(x7_meas[5]), w=float(x7_meas[6]),
...
def main():
    ...
    params = Params()
    print("=" * 70)
    print(" Quad-X 耐故障 NMPC シミュレーション")
    print("=" * 70)
    print(f"  状態次元 NX={params.NX} (NMPC), プラント=7 (φ,θ,p,q,r,z,w)")
    ...
    print(f"  ホバー duty (健全4機) u_hover_nom = {params.u_hover_nom:.4f}")
    print(f"  機体: Quad-X (M1=FR/CCW, M2=RL/CCW, M3=FL/CW, M4=RR/CW)")
    print(f"  冗長性: null space = NU - 3 = 1 次元 (平均モード = 高度成分のみ)")
    print(f"  → 完全故障 (η=0) は幾何的に対処不可、部分故障 (η>0) でのみ FTC 可能")
    ...
    # 4) 故障+5 s で phi_ref=0.2 rad ステップ
    HOVER_Z_2M = params.z_ref_default
    T_SIM = 15.0       # ascent ~3s + 切替+2s + step+5s + 余裕
    x0_ground = np.zeros(7)
    PLAN_PHI_STEP = (5.0, 0.2)
    FAULT_DELAY_S = 2.0
    ...
    # Quad-X 専用シナリオ (完全故障は幾何的に対処不可なので部分故障中心に再編)
    # モータ番号: M1=FR(idx 0), M2=RL(idx 1), M3=FL(idx 2), M4=RR(idx 3)
    results['S1'] = run_ftc_plan(
        "S1: healthy (no fault)",
        faults=[], fdi_known=True, png_name="sim_s1_healthy.png")

    # S2 系: M3 (前左, CW) の部分故障 η=0.7
    results['S2a'] = run_ftc_plan(
        "S2a: M3 partial η=0.7, FDI KNOWN",
        faults=[(2, 0.7)], fdi_known=True, png_name="sim_s2a_partial_70_known.png")

    results['S2b'] = run_ftc_plan(
        "S2b: M3 partial η=0.7, FDI UNKNOWN",
        faults=[(2, 0.7)], fdi_known=False, png_name="sim_s2b_partial_70_unknown.png")

    # S3 系: より厳しい部分故障 (η=0.5、クアッドの実用限界に近い)
    results['S3a'] = run_ftc_plan(
        "S3a: M3 partial η=0.5, FDI KNOWN",
        faults=[(2, 0.5)], fdi_known=True, png_name="sim_s3a_partial_50_known.png")

    results['S3b'] = run_ftc_plan(
        "S3b: M3 partial η=0.5, FDI UNKNOWN",
        faults=[(2, 0.5)], fdi_known=False, png_name="sim_s3b_partial_50_unknown.png")

    # S4: 厳しい部分故障 (η=0.3、追従性能の限界デモ)
    results['S4'] = run_ftc_plan(
        "S4: M3 severe partial η=0.3, FDI KNOWN",
        faults=[(2, 0.3)], fdi_known=True, png_name="sim_s4_partial_30_known.png")

    # S5: 完全故障 (η=0) → 制御不能のデモ (敢えて残して限界を示す)
    results['S5'] = run_ftc_plan(
        "S5: M3 complete fault η=0 (geometrically uncontrollable demo)",
        faults=[(2, 0.0)], fdi_known=True, png_name="sim_s5_complete_demo.png")
    ...
    results['S8'] = run_ftc_plan(
        "S8: M3 partial η=0.5 + torque gust",
        faults=[(2, 0.5)], fdi_known=True,
